...
func TestSimpleTypeSpec(t *testing.T) {
	...
	t.Run("001", func(t *testing.T) {
		...
		assert.Equal(t, "unsigned long", typeDeclarator.TypeSpec().GetName())
	...
	t.Run("002", func(t *testing.T) {
		stream := fmt.Sprintf(`
			typedef unsigned long type1, type2; 
		`)
		controller := gomock.NewController(t)
		defer controller.Finish()
		reader := bufio.NewReader(strings.NewReader(stream))
		...
		assert.Equal(t, ScopingInterfaces.RWunsignedRWlongIdlType, typeDeclarator.TypeSpec().GetKind())
		spec, _ = spec.GetNextTypeSpec()
		assert.Nil(t, spec)
	...
	t.Run("002 in two module", func(t *testing.T) {
		stream := fmt.Sprintf(`
			...
			module DEF
			{
				typedef unsigned long type3, type4;
			};
		`)
		controller := gomock.NewController(t)
		defer controller.Finish()
